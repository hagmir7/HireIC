...
import WorkPostForm from '../components/WorkPostForm'
...
const WorkPost = () => {
    ...
    const handleDelete = async (work_post_id) => {
        try {
            await api.delete(`posts/${work_post_id}`)
    ...
    const handleMenuClick = (key, id) => {
        switch (key) {
            case "edit":
                setWorkPost(data.find((item) => Number(item.id) === Number(id)))
                setIsModalOpen(true)
                break;
            case 'delete':
        ...
        <div className='h-full flex flex-col'>
            <div className='shadow-sm p-2 flex items-center justify-between from-gray-50 to-gray-100 border-b'>
                ...
                <div className='flex gap-3'>
                    ...
                    >
                        <WorkPostForm
                            initialValues={workPost}
                            onSubmit={async (data) => {
                                if (workPost) {
                                    await api.put(`posts/${workPost.id}`, data);
                                } else {
            ...
            <div className='flex-1 overflow-y-auto'>
                <table className='w-full border-collapse'>
                    ...
                    <tbody className='bg-white'>
                        ...
                        ) : (<TableEmpty description="Aucun Postes de travail trouvé." colSpan={3} Create={setIsModalOpen} />)}
                    </tbody>
                </table>
            </div>
