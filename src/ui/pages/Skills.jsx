...
const Skills = () => {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const res = await api.get('skills/type')
      setData(res.data)
    } catch (error) {
  ...
  const handleDelete = async (diplome_id) => {
    try {
      await api.delete(`skills/type/${diplome_id}`)
          ...
          >
           <SkillTypeForm
                ...
                onSubmit={async (data) => {
                    try {
                    if (level) {
                        await api.put(`skills/type/${level.id}`, data);
                        message.success("Niveau mis à jour avec succès !");
                    } else {
                        await api.post("skills/type", data);
      ...
      <div className='flex-1 overflow-y-auto'>
        <table className='w-full border-collapse'>
          ...
          <tbody className='bg-white'>
            ...
            ) : ( <TableEmpty colSpan={4} description="Aucun Compétence trouvé." Create={setIsModalOpen}  />)}
          </tbody>
        </table>
      </div>
